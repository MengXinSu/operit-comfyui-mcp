...
3. 让 AI 在 ComfyUI 中搭建工作流，或加载已有工作流
4. 调 comfy_generate 改提示词，开始生图

> 首次生图较慢（ComfyUI 需编译 Shader），之后就快了。

## 💡 游玩建议

可以让 AI 为你写一个 Operit skill，把常用的工作流、模型、分辨率、步数等参数预设好。
之后每次生图只需要 `@你的skill 描述画面` 就能一键出图，不用每次都问参数。

服务器地址（IPv4 地址）可能会变，每次使用时仍需告诉 AI 当前地址。
